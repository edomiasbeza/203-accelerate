...
?>
<?php 
  $vw_painter_archive_year  = get_the_time('Y'); 
  $vw_painter_archive_month = get_the_time('m'); 
  $vw_painter_archive_day   = get_the_time('d'); 
?>
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
  <div class="post-main-box">
    <?php $vw_painter_theme_lay = get_theme_mod( 'vw_painter_blog_layout_option','Default');
    if($vw_painter_theme_lay == 'Default'){ ?>
      <div class="row m-0">
        ...
        <div class="new-text <?php if(has_post_thumbnail()) { ?>col-lg-6 col-md-6"<?php } else { ?>col-lg-12 col-md-12"<?php } ?>>
          ...
          <?php if( get_theme_mod( 'vw_painter_toggle_postdate',true) != '' || get_theme_mod( 'vw_painter_toggle_author',true) != '' || get_theme_mod( 'vw_painter_toggle_comments',true) != '') { ?>
            <div class="post-info">
              <?php if(get_theme_mod('vw_painter_toggle_postdate',true)==1){ ?>
                <i class="fas fa-calendar-alt"></i><span class="entry-date"><a href="<?php echo esc_url( get_day_link( $vw_painter_archive_year, $vw_painter_archive_month, $vw_painter_archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span><span>|</span>
              <?php } ?>
              ...
              <?php if(get_theme_mod('vw_painter_toggle_comments',true)==1){ ?>
                <i class="fa fa-comments" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'vw-painter'), __('0 Comments', 'vw-painter'), __('% Comments', 'vw-painter') ); ?> </span>
              <?php } ?>
              <hr>
            </div>   
          <?php } ?>   
          <div class="entry-content">
            <p>
              <?php $vw_painter_theme_lay = get_theme_mod( 'vw_painter_excerpt_settings','Excerpt');
              if($vw_painter_theme_lay == 'Content'){ ?>
                <?php the_content(); ?>
              <?php }
              if($vw_painter_theme_lay == 'Excerpt'){ ?>
    ...
    <?php }else if($vw_painter_theme_lay == 'Center'){ ?>
    ...
    <?php }else if($vw_painter_theme_lay == 'Left'){ ?>
      <div class="service-text">
        <div class="box-image">
          <?php the_post_thumbnail(); ?>
        </div>
